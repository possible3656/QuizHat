...
import com.winbee.quizhat.Adapter.ThirdLevelTestListAdapter;
...
public class ThirdLevelActivity extends AppCompatActivity {
    ...
    private void getTestList() {
        ...
        Call<TLDetailsMainModel> call = apiClient.fetchTLDetails(OnlineTestData.org_code, OnlineTestData.auth_code2, UserId,
                OnlineTestData.bucketIDThirdLevel, "1", "L3");
        call.enqueue(new Callback<TLDetailsMainModel>() {
            @Override
            public void onResponse(Call<TLDetailsMainModel> call, Response<TLDetailsMainModel> response) {
                apiCalled();
                TLDetailsMainModel siacDetailsMainModel = response.body();
                if (siacDetailsMainModel != null) {
                    if (siacDetailsMainModel.getMessage().equalsIgnoreCase("true")) {
                        List<TLDetailsDataModel> siacDetailsDataModelList = new ArrayList<>(Arrays.asList(siacDetailsMainModel.getData()));
                        if (siacDetailsDataModelList.isEmpty()) {
                            txt_nodata.setVisibility(View.VISIBLE);
                        } else {

                            txt_nodata.setVisibility(View.GONE);
                            ThirdLevelTestListAdapter testListAdapter = new ThirdLevelTestListAdapter(ThirdLevelActivity.this, siacDetailsDataModelList);
                            RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(ThirdLevelActivity.this, LinearLayoutManager.VERTICAL, false);
            ...
            @Override
            public void onFailure(Call<TLDetailsMainModel> call, Throwable t) {
    ...
    private void doToast(String msg) {
        ...
        toast_msg = Toast.makeText(ThirdLevelActivity.this, msg, Toast.LENGTH_SHORT);
        toast_msg.show();
    }
}
